...
public class NPC : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        optionButton1 = DialogSystem.Instance.option1Button;
        optionButton1Text = DialogSystem.Instance.option1Button.transform.Find("Text (TMP)").GetComponent<TextMeshProUGUI>();
            ...
            if (currentActiveQuest.accepted && currentActiveQuest.isCompleted == false)
            {
                if (AreQuestRequirementsCompleted())
                {
                    ...
                    optionButton1.onClick.AddListener(() => {
                        ReceiveRewardAndCompleteQuest(); /// will set a new quest here if there is one.
                    });
                }
                else
                {
